...
dplyr::filter(YEAR >= 1992, YEAR <= 2024) |>
dplyr::group_by(YEAR) |>
dplyr::summarise(
...
Checking the issue in 2004 

```{r}
WeatherData |>
  dplyr::filter(YEAR == 2004) |>
  dplyr::select(YEARMODA, PRCP) |>
  head(50) 


summary(WeatherData$PRCP[WeatherData$YEAR == 2004])

```

```{r}
rain_2004 <- WeatherData |>
  dplyr::filter(YEAR == 2004)

nrow(rain_2004)

dplyr::slice_head(rain_2004, n = 10) |> dplyr::select(YEARMODA, PRCP)
dplyr::slice_tail(rain_2004, n = 10) |> dplyr::select(YEARMODA, PRCP)

# rainfall in 2004
summary(rain_2004$PRCP)
sum_2004 <- sum(rain_2004$PRCP, na.rm = TRUE)
sum_2004

```

```{r}
check_rain <- WeatherData |>
  dplyr::filter(YEAR != 2004)

summary(check_rain$PRCP)

quantile(check_rain$PRCP, c(.5, .9, .95, .99), na.rm = TRUE)
quantile(rain_2004$PRCP,  c(.5, .9, .95, .99), na.rm = TRUE)

```

I think that the units for 2004 might be inconsistent, some rainfall events are super inflated like 149mm, should possibly be 14.9mm? 
I think its best to exclude 2004 from the analysis
...
```{r}
rain_yearly <- weather_yearly |> dplyr::filter(YEAR != 2004)
```


```{r}
ggplot(weather_yearly, aes(YEAR, total_rain)) +
  geom_line() +
  geom_point() +
  theme_bw() +
  labs(x = "Year", y = "Total annual rainfall (mm)")
...
### Look at daily temps (For lagged effects too its nb
