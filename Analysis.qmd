...
# IMPORTANT: labels must match your cand_obj3 names (no maxT/maxR/maxW/hailD unless in cand_obj3)
...
# add post hoc pop-size models if they exist
...
doc <- read_docx() |>
  body_add_par(
    paste0(
      "Table X. Model selection results for breeding timing (annual median laying date, day of year). ",
      "Weather cue predictors were summarised for a fixed March window (01–31 March) each year; lagged predictors represent annual conditions in the previous year. ",
      "Models were fitted using linear regression. K = number of parameters; Deviance = -2 log-likelihood; AICc = small-sample AIC; ΔAICc = difference from the best model; w = Akaike weight; R² = variance explained. ",
      "Slope (SE) is shown for single-predictor models only. Population size models (if included) were fitted post hoc and are shown for comparison."
    ),
    style = "Normal"
  ) |>
  body_add_flextable(ft)
